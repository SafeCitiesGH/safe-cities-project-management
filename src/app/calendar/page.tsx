...
import { Suspense, useCallback, useMemo, useState, useEffect, type FormEvent } from 'react'
...
function CalendarPageContent() {
    ...
    try {
      ...
      if (data.googleError) {
        toast({
          ...
          description:
            'Your own events are shown. Reconnect Google to see its events again.',
          variant: 'destructive',
        })
      }
  ...
  // After a successful connect, offer to move events the user already created
  ...
  useEffect(() => {
    ...
    if (googleState !== 'connected' && googleState !== 'reconnected') {
      return
    }
    ...
    async function offerSync() {
      try {
        ...
        if (data.googleConnected && data.pending > 0) {
          setPendingSyncCount(data.pending)
          setIsSyncPromptOpen(true)
        }
      } catch (err) {
        console.error('Error checking pending events:', err)
      }
    }
  ...
  const handleSyncToGoogle = async () => {
    ...
    try {
      ...
      toast({
        title: `Added ${data.pushed ?? 0} event${data.pushed === 1 ? '' : 's'} to Google Calendar`,
        description: data.failed
          ? `${data.failed} event${data.failed === 1 ? '' : 's'} could not be added.`
          : undefined,
      })
      ...
      toast({
        title: 'Failed to add events to Google Calendar',
        description: err instanceof Error ? err.message : 'Unknown error',
        variant: 'destructive',
      })
    } finally {
  ...
  const openAddEventDialog = (date: Date) => {
    ...
    setEnd(range.end)
    setAddToGoogle(googleStatus.connected)
    setLastMessage(null)
    ...
    try {
      const response = await fetch('/api/calendar/events', {
        ...
        body: JSON.stringify({
          ...
          addToGoogle: googleStatus.connected && addToGoogle,
        }),
      })
      ...
      if (data.googleSyncFailed) {
        toast({
          title: 'Event saved, but not added to Google',
          description:
            'The event is saved here. Reconnect Google Calendar to sync it.',
          variant: 'destructive',
        })
      } else {
  ...
  const handleDeleteEvent = async (eventId: string) => {
    ...
    try {
      ...
      let data: { error?: string; message?: string; googleDeleteFailed?: boolean } | null = null
      try {
      ...
      if (data?.googleDeleteFailed) {
        toast({
          title: 'Event deleted here, but not in Google',
          description: 'The copy in Google Calendar could not be removed.',
          variant: 'destructive',
        })
      } else {
    ...
    <div className="flex flex-col h-screen bg-background">
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground">
                  {googleStatus.connected
                    ? `Google Calendar connected as ${googleStatus.googleEmail ?? 'Google account'}`
                    : 'Your events are saved here. Connect Google Calendar to sync them.'}
                </p>
              </div>
            </CardHeader>
      ...
      <AlertDialog open={isSyncPromptOpen} onOpenChange={setIsSyncPromptOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              Add your existing events to Google Calendar?
            </AlertDialogTitle>
            <AlertDialogDescription>
              You have {pendingSyncCount} event{pendingSyncCount === 1 ? '' : 's'} created
              before connecting Google. Adding them copies them into your Google
              Calendar. They stay here either way.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={isSyncing}>Not now</AlertDialogCancel>
            <AlertDialogAction
              onClick={(e) => {
                e.preventDefault()
                void handleSyncToGoogle()
              }}
              disabled={isSyncing}
            >
              {isSyncing ? 'Adding...' : 'Add to Google Calendar'}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Add a new event for the selected date.</DialogDescription>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {googleStatus.connected ? (
              <div className="flex items-center gap-2">
                <Checkbox
                  id="event-add-to-google"
                  checked={addToGoogle}
                  onCheckedChange={(checked) => setAddToGoogle(checked === true)}
                />
                <Label htmlFor="event-add-to-google" className="font-normal">
